...
/* Implements multiplication and REDC for two input numbers of 10 words */
...
// local variables: tmp[0 ... 10] array, having 10+1 8-byte words
// The tmp array needs 10+1 entries, but tmp[10] is stored in
// r15, so only 10 entries are used in the stack.
...
// Pass for j = 0. We need to fetch x[i] from memory and compute the new u
	...
	subi    r1, r1, 80		// set tmp stack space
...
// Pass for j = 9. Don't fetch new data from y[j+1].
	...
	addc    r8, r8, r13		// add T0 and low word
	adde    r13, r9, r14		// add high word with carry to T1
	std     r8, 64(r1)		// store tmp[len-2]
	addze   r15, r16		// put carry in r15 (tmp[len] <= 1)
	std     r13, 72(r1)		// store tmp[len-1]
