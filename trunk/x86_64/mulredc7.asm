...
/* Implements multiplication and REDC for two input numbers of 7 words */
...
# Pass for j = 6. Don't fetch new data from y[j+1].
	...
	mulq    %r11		# m[j]*u
	addq	%rax, %rsi	# Add low word to T0
	movq	%rsi, 40(%rbp)	# Store T0 in tmp[j-1]
	adcq	%rdx, %rbx	# Add high word with carry to T1
	movq	%rbx, 48(%rbp)	# Store T1 in tmp[j]
	setc	%cl		# %CY <= 1
	movq	%rcx, 56(%rbp)	# Store CY in tmp[j+1]
