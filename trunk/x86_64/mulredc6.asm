...
# Pass for j = 5. Don't fetch new data from y[j+1].
	...
	mulq    %r11		# m[j]*u
	addq	%rax, %rbx	# Add low word to T0
	movq	%rbx, 32(%rbp)	# Store T0 in tmp[j-1]
	adcq	%rdx, %rsi	# Add high word with carry to T1
	movq	%rsi, 40(%rbp)	# Store T1 in tmp[j]
	setc	%cl		# %CY <= 1
	movq	%rcx, 48(%rbp)	# Store CY in tmp[j+1]
...
# Pass for j = 5. Don't fetch new data from y[j+1].
	...
	mulq    %r11		# m[j]*u
	addq	%rax, %rsi	# Add low word to T0
	movq	%rsi, 32(%rbp)	# Store T0 in tmp[j-1]
	adcq	%rdx, %rbx	# Add high word with carry to T1
	movq	%rbx, 40(%rbp)	# Store T1 in tmp[j]
	adcb	$0, %cl	# %CY <= 2
	movq	%rcx, 48(%rbp)	# Store CY in tmp[j+1]
