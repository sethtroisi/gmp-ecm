    ...
    int monic, mpzspm_t mpzspm)
{
  ...
  mpzspv_mul_ntt(v, 0, 
                 v, 0, len, 
                 NULL, UNUSED, UNUSED, 
                 2 * len, monic, UNUSED, mpzspm, NTT_MUL_STEP_FFT1);
  mpzspv_mul_ntt(u, 0, 
                 u, 0, len, 
                 v, 0, 2*len, 
                 2 * len, monic, monic ? 2 * len : 0, 
                 mpzspm, NTT_MUL_STEP_FFT1 + NTT_MUL_STEP_MUL + NTT_MUL_STEP_IFFT);
  ...
  for (; m < m_max; m *= 2)
    {
	  ...
	  mpzspv_from_mpzv (x, i + 2 * m, src + i / 2 + m, m, mpzspm);
          mpzspv_mul_ntt (x, i + 2 * m, 
                          x, i + 2 * m, m, 
                          NULL, UNUSED, UNUSED, 
                          2 * m, 1, UNUSED, mpzspm, NTT_MUL_STEP_FFT1);
          mpzspv_mul_ntt (x, i, 
                          x, i, m, 
                          x, i + 2 * m, 2*m, 
                          2 * m, 1, 2 * m, mpzspm,
  ...
  for (; k < len; k *= 2)
    {
      ...
      mpzspv_mul_ntt (x, 0, 
                      x, 0, k + 1, 
                      NULL, UNUSED, UNUSED, 
                      2 * k, 0, 0, mpzspm, NTT_MUL_STEP_FFT1);
      mpzspv_mul_ntt (z, 0, 
                      z, 0, 2 * k - 1, 
                      x, 0, 2 * k, 
                      2 * k, 0, 0, mpzspm, 
  ...
  for (m = len / 2; m >= POLYEVALT_NTT_THRESHOLD; m /= 2)
    {
	  ...
	  mpzspv_mul_ntt (y, i, 
	                  y, i, 2 * m, 
	                  NULL, UNUSED, UNUSED, 
	                  2 * m, 0, 0, mpzspm, NTT_MUL_STEP_FFT1);
          mpzspv_mul_ntt (x, 0, 
                          x, 0, m + 1, 
                          y, i, 2 * m, 
                          2 * m, 0, 0, mpzspm, 
