...
__device__ void Cuda_Fully_Normalize_64(biguint_t A,dbigint_t cy)
{
  ...
  }while(__any(cy[threadIdx.x]|cy[threadIdx.x+NB_DIGITS])!=0);
}
*/
...
__device__ void Cuda_RedMont_Step (dbiguint_t r, dbigint_t cy)
{
  ...
  r[NB_DIGITS+threadIdx.x]=r[NB_DIGITS+threadIdx.x+1];
  cy[NB_DIGITS+threadIdx.x]=cy[NB_DIGITS+threadIdx.x+1];
  if (threadIdx.x==0)
  {
    cy[2*NB_DIGITS-1]=0;
    r[2*NB_DIGITS-1]=0;
...
{
  __shared__ int b_cy[CURVES_BY_BLOCK][2*NB_DIGITS]; 
  __shared__ unsigned int b_r[CURVES_BY_BLOCK][2*NB_DIGITS]; 
  __shared__ unsigned int b_b[CURVES_BY_BLOCK][NB_DIGITS]; 
  ...
  r[NB_DIGITS+threadIdx.x]=Aarg[idx1][threadIdx.x];
  b[threadIdx.x]=Barg[idx1][threadIdx.x];
    
  cy[threadIdx.x]=0;
  cy[threadIdx.x+NB_DIGITS]=0;

  Cuda_Mul_mod(r,cy,r+NB_DIGITS,b,r);
...
__global__ void Cuda_Test(biguint_t *Aarg, biguint_t *Barg)
{
  __shared__ unsigned int r[CURVES_BY_BLOCK][2*NB_DIGITS];
  __shared__ int cy[CURVES_BY_BLOCK][2*NB_DIGITS]; 
  __shared__ unsigned int A[CURVES_BY_BLOCK][NB_DIGITS]; 
  __shared__ unsigned int B[CURVES_BY_BLOCK][NB_DIGITS]; 
  unsigned int idx1=blockIdx.x*blockDim.y+threadIdx.y;
  int i;

  //init
  cy[threadIdx.y][threadIdx.x]=0; 
  cy[threadIdx.y][NB_DIGITS + threadIdx.x]=0; 
  r[threadIdx.y][threadIdx.x]=Barg[idx1][threadIdx.x];
  r[threadIdx.y][NB_DIGITS + threadIdx.x]=0;  
