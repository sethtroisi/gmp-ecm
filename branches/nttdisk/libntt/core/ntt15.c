...
static void
ntt15_run(spv_t in, spv_size_t istride, spv_size_t idist,
    		spv_t out, spv_size_t ostride, spv_size_t odist,
    		spv_size_t num_transforms, sp_t p, spv_t ntt_const)
{
  ...
  for (; i < num_transforms; i++)
    ntt15_run_core(in + i * idist, istride, 
...
#ifdef HAVE_SIMD
static void
ntt15_run_simd(spv_t in, spv_size_t istride, spv_size_t idist,
    		spv_t out, spv_size_t ostride, spv_size_t odist,
    		spv_size_t num_transforms, sp_t p, spv_t ntt_const)
{
  spv_size_t i = 0;
  ...
  for (; i < num_simd; i += SP_SIMD_VSIZE)
    ntt15_run_core_simd(in + i * idist, istride, idist, 
...
static void
ntt15_twiddle_run(spv_t in, spv_size_t istride, spv_size_t idist,
    			spv_t out, spv_size_t ostride, spv_size_t odist,
    			spv_t w, spv_size_t num_transforms, sp_t p, spv_t ntt_const)
{
  ...
  for (; i < num_transforms; i++, j += 2*(15-1))
    ntt15_twiddle_run_core(in + i * idist, istride, 
			out + i * odist, ostride,
			w + j, p, ntt_const);
}
...
#ifdef HAVE_SIMD
static void
ntt15_twiddle_run_simd(spv_t in, spv_size_t istride, spv_size_t idist,
    			spv_t out, spv_size_t ostride, spv_size_t odist,
    			spv_t w, spv_size_t num_transforms, sp_t p, spv_t ntt_const)
{
  spv_size_t i = 0, j = 0;
...
#ifdef HAVE_SIMD
static void
ntt15_pfa_run_simd(spv_t x, spv_size_t cofactor,
	  sp_t p, spv_t ntt_const)
{
  spv_size_t i = 0;
  spv_size_t incstart = 0;
  ...
  for (i = 0; i < num_simd; i += SP_SIMD_VSIZE)
    {
      ntt15_pfa_run_core_simd(x, incstart, inc, inc2, n, p, 
	  ntt_const, SP_SIMD_VSIZE);
