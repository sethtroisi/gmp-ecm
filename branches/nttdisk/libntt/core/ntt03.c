...
static void
ntt3_run(spv_t x, spv_size_t num_transforms,
	  sp_t p, spv_t ntt_const)
{
  spv_size_t i = 0;
  ...
  for (i = 0; i < num_simd; i += 3 * SP_SIMD_VSIZE)
    ntt3_run_core_simd(x + i, 1, 3, x + i, 1, 3, p, ntt_const);
#endif

  for (; i < num_transforms; i += 3)
    ntt3_run_core(x + i, 1, x + i, 1, p, ntt_const);
}


static void
ntt3_twiddle_run_core(spv_t x, spv_t w, spv_size_t stride,
			sp_t p, spv_t ntt_const)
{
  sp_t p0, p1, p2;
  sp_t x0, x1, x2;
...
#ifdef HAVE_SSE2
static void
ntt3_twiddle_run_core_simd(spv_t x, sp_simd_t *w,
			spv_size_t stride,
			sp_t p, spv_t ntt_const)
{
  sp_simd_t p0, p1, p2;
...
static void
ntt3_twiddle_run(spv_t x, spv_t w,
	  spv_size_t stride,
	  ...
	  sp_t p, spv_t ntt_const)
{
  ...
  for (i = 0; i < num_simd; i += SP_SIMD_VSIZE,
    ...
    ntt3_twiddle_run_core_simd(x + i, (sp_simd_t *)(w + j),
				stride, p, ntt_const);
#endif

  for (; i < num_transforms; i++, j += 2*(3-1))
    ntt3_twiddle_run_core(x + i, w + j, stride, p, ntt_const);
}
