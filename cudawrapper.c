...
gpu_ecm (mpz_t f, ecm_params params, mpz_t n, double B1)
{
  ...
  size_t max_bits = (params->gpu_cgbn ? ECM_GPU_CGBN_MAX_BITS : ECM_GPU_MAX_BITS) - 6;
  ...
  int schedule = params->gpu_cgbn ? 1 : 0;
  ...
  if (params->gpu_cgbn) {
    ...
    youpi = gpu_ecm_stage1 (factors, array_found, n, params->batch_s, nb_curves,
                            firstsigma_ui, &gputime, params->verbose);
  }
  ...
  for (i = 0; i < nb_curves; i++)
    {
      mpz_mul_2exp (params->x, params->x, n_bits);
      mpz_add (params->x, params->x, factors[nb_curves - 1 - i]);
    }
