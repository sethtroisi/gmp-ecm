            ...
            mpz_t go, int (*stop_asap)(void), char *chkfilename)
{
  ...
  /* Compute a^(p_1^e_1 * p_2^e_2 * ...) iteratively.
     Batch primes with exponent >2 into a first large batch.
     Batch primes with exponent =2 into a second batch.
     Then batch primes into "chunks" of around X bits each. */
